...
def load_bank_accounts(f):
    ...
    def decorated_function(*args, **kwargs):
        if 'email' in session:
            ...
            ### You may want to hardcode the email address for demo purposes
            # counterparty =  modern_treasury.counterparties.list(metadata={ "email": "kevin.krier@gmail.com"})
...
def create_user_ledger_accounts_and_categories(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        check_categories =  modern_treasury.ledger_account_categories.list(
            metadata=metadata_filters,
            ledger_id=current_app.config['ledger'].id
        ).items
